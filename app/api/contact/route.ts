...
export async function POST(req: Request) {
  const { name, email, company, who, vertical, message } = await req.json()

  await resend.emails.send({
    from: 'Book an Intro <klymenko0105@gmail.com>',
    to: 'eugene@aiplusvibe.com',
    replyTo: email,
    ...
    html: `
      <div style="font-family:monospace;max-width:600px;margin:0 auto;padding:32px">
        ...
        <table style="width:100%;border-collapse:collapse">
          ...
          <tr><td style="padding:8px 0;color:#888">They are</td><td style="padding:8px 0">${who || '—'}</td></tr>
